module CE
  class Auth
    ...
    def self.login_with_token(token, user_id)
      redis = Redis.new
      redis_key = CE::Redis::Keys.auth_token(token)
    ...
    def self.create_account(token, email, password)
      ...
      redis = Redis.new
      redis_key = CE::Redis::Keys.new_account_password_link(token)
    ...
    def self.reset_password(token, user_id, password)
      redis = Redis.new
      redis_key = CE::Redis::Keys.reset_password_link(token)
    ...
    def self.clear_user_fails(user_id)
      redis = Redis.new
      redis_key = CE::Redis::Keys.user_auth_fail(user_id)
      redis.del redis_key
    end
    ...
    def self.user_blocked?(user_id)
      redis = Redis.new
      key = CE::Redis::Keys.user_auth_fail(user_id)
